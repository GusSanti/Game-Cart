...
local create_speed_effect: (Model) -> ()
...
local function update_overdrive_effects(): ()
	...
	local shouldEnableEffects = is_vfx_enabled() and isOverdriveActive == true
	if shouldEnableEffects and #overdriveEmitters == 0 then
		for _, sparkEffect in sparkEffects do
	...
	for _, emitter in overdriveEmitters do
		emitter.Enabled = shouldEnableEffects
	end
...
local function update_vfx_state(): ()
	if not is_vfx_enabled() then
		clear_spark_effects()
	elseif activeCharacter then
		if #sparkEffects == 0 then
			create_spark_effects(activeCharacter)
		end
		if #speedEmitters == 0 then
			create_speed_effect(activeCharacter)
		end
	end
...
create_speed_effect = function(character: Model): ()
	local rootPart = character:FindFirstChild("HumanoidRootPart")
	if not rootPart or not rootPart:IsA("BasePart") then
		return
	end
