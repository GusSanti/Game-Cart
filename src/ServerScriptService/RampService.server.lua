...
local function update_active_slides(deltaTime: number): ()
	...
	for player, slide in activeSlidesByPlayer do
		...
		local wasOverdriveActive = slide.character:GetAttribute(CART_OVERDRIVE_ACTIVE_ATTRIBUTE) == true
		update_slide_resources(slide, modeState, currentTime, slideDeltaTime)
		if not wasOverdriveActive and slide.character:GetAttribute(CART_OVERDRIVE_ACTIVE_ATTRIBUTE) == true then
